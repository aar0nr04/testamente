...
export async function createRegistrationProfile(uid: string, data: Omit<UserProfile, 'uid' | 'createdAt' | 'updatedAt'>): Promise<void> {
  await setDoc(doc(db, 'users', uid), { ...data, createdAt: serverTimestamp(), updatedAt: serverTimestamp(), isApprovedPsychologist: false, 'professional.isVerified': false }, { merge: false });
}
