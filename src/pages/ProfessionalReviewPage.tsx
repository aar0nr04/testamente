...
export function ProfessionalReviewPage() {
  ...
  return <section className="stack"><div className="page-heading"><div><span className="eyebrow">Área protegida</span><h1>Revisión profesional</h1><p>Los estados clínicos cambian sólo con evidencia documentada. Sesión: {profile?.role ?? user?.uid}.</p></div></div><div className="panel table-scroll"><table className="catalog-status-table"><thead><tr><th>Instrumento</th><th>Categoría</th><th>Licencia</th><th>Técnico</th><th>Clínico</th><th>ES</th><th>EN</th><th>IT</th><th>FR</th><th>DE</th><th>ZH</th><th>PT</th><th>Revisión</th></tr></thead><tbody>{instrumentRegistry.map((instrument) => <tr key={instrument.id}><td>{instrument.abbreviation}</td><td>{instrument.subcategory}</td><td>{instrument.licenseStatus}</td><td>{instrument.scoringStatus}</td><td>{instrument.clinicalReviewStatus}</td>{(['es', 'en', 'it', 'fr', 'de', 'zh', 'pt'] as const).map((language) => <td key={language}>{instrument.localeStatus[language]}</td>)}<td>{instrument.lastReviewedAt ?? '—'}</td></tr>)}</tbody></table></div><div className="panel stack"><label>Instrumento para revisión<select value={selectedId} onChange={(event) => { setSelectedId(event.target.value); setPayload(undefined); setError(''); }}>{instrumentRegistry.map((instrument) => <option key={instrument.id} value={instrument.id}>{instrument.abbreviation} — {instrument.fullName}</option>)}</select></label>{selected ? <p className="muted">{selected.notes}</p> : null}<div className="row-gap"><button disabled={loading} onClick={() => void loadPayload()}>{loading ? 'Cargando…' : 'Cargar para revisión'}</button></div>{error ? <p className="error" role="alert">{error}</p> : null}{activePayload ? <ReviewRunner payload={activePayload} reviewerId={user?.uid ?? ''} /> : <p className="muted">No hay contenido cargado. Los instrumentos restringidos requieren un payload autorizado y una allowlist de backend.</p>}</div></section>;
}
...
function ReviewRunner({ payload, reviewerId }: { payload: PrivateReviewPayload; reviewerId: string }) {
  ...
  async function saveReview() {
    ...
    try { await addDoc(collection(requireFirestore(), 'testProfessionalReviews'), { instrumentId: payload.instrumentId, contentVersion: payload.contentVersion, algorithmVersion: payload.algorithmVersion, locale: payload.locale, reviewerId, status, questionComments: {}, scoringComments: score ? comments : '', interpretationComments: '', translationComments: '', generalComments: comments, createdAt: serverTimestamp(), updatedAt: serverTimestamp() }); setMessage('Revisión guardada.'); }
