...
export function AppointmentsPage() {
  ...
  if (!user) return null;
  const uid = user.uid;
  async function request() { if (!psychologistId || !startTime) { setMessage('Completa profesional y fecha.'); return; } try { await addDoc(collection(requireFirestore(), 'appointments'), { patientId: uid, psychologistId, participantIds: [uid, psychologistId], status: 'pending', proposedStartTime: new Date(startTime), timezone: Intl.DateTimeFormat().resolvedOptions().timeZone, notes, createdAt: serverTimestamp(), updatedAt: serverTimestamp() }); setMessage('Solicitud enviada.'); } catch { setMessage('No se pudo crear la solicitud.'); } }
