export type AppCheckProvider = 'v3' | 'enterprise';
...
export interface AppEnv {
  ...
  functionsRegion: string; appCheckSiteKey?: string; appCheckProvider: AppCheckProvider; aiTipsBaseUrl?: string; useFirebaseEmulators: boolean; amasPublicEnabled: boolean; amasReviewEnabled: boolean;
}
...
export function resolveAppCheckProvider(value: string | undefined): AppCheckProvider {
  if (!value || value === 'v3') return 'v3';
  if (value === 'enterprise') return 'enterprise';
  throw new Error('VITE_APPCHECK_PROVIDER debe ser "v3" o "enterprise".');
}
...
export const appEnv: AppEnv = { firebase, functionsRegion: read('VITE_FIREBASE_FUNCTIONS_REGION') ?? 'us-central1', appCheckSiteKey: read('VITE_FIREBASE_APPCHECK_SITE_KEY'), appCheckProvider: resolveAppCheckProvider(read('VITE_APPCHECK_PROVIDER')), aiTipsBaseUrl: read('VITE_AI_TIPS_BASE_URL'), useFirebaseEmulators: enabled('VITE_USE_FIREBASE_EMULATORS'), amasPublicEnabled: enabled('VITE_AMAS_PUBLIC_ENABLED'), amasReviewEnabled: enabled('VITE_AMAS_REVIEW_ENABLED') };
